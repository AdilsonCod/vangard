  }, [parsedData, users, monthlyBarberStats, selectedMonth, selectedYear, manualUserMapping]);
      ...
      if (importType === 'DPOTE_PDF' && dpoteReport) {
        ...
        for (const item of parsedData) {
          ...
          if (item.categoriaNome) {
             let cat = currentCategories.find(c => c.name.toLowerCase() === item.categoriaNome.toLowerCase());
             if (!cat) {
               catId = 'cat_' + Date.now() + Math.random().toString(36).substring(2, 9);
               const newCat = {
                 id: catId,
          ...
          const existingItemIndex = currentCatalog.findIndex(c => c.name.toLowerCase() === item.itemNome.toLowerCase());
          if (existingItemIndex >= 0) {
             ...
             currentCatalog.push({
               id: 'item_' + Date.now() + Math.random().toString(36).substring(2, 9),
               name: item.itemNome,
               type: catId || '',
        ...
        for (const group of groupedData) {
          ...
          const current = monthlyUnitStats.find(s => s.id === statId) || {
            id: statId,
            unitId: group.unitId,
            month: monthStr,
            faturamentoTotal: 0,
          ...
          for (const item of group.items) {
             const catItem = catalog.find(c => c.name.toLowerCase() === item.itemNome.toLowerCase());
        ...
        for (const group of groupedData) {
          ...
          const updated = {
            ...
            faturamentoAssinatura: current.faturamentoAssinatura + group.faturamentoAssinatura,
            assinantes: group.assinantes > 0 ? group.assinantes : current.assinantes,
            clientesNovos: current.clientesNovos + group.clientesNovos,
            clientesAtendidos: current.clientesAtendidos + group.clientesAtendidos,
            servicosRealizados: current.servicosRealizados + group.servicosRealizados
          };
        ...
        const current = monthlyBarberStats.find(s => s.id === statId) || {
          id: statId,
        ...
        const updated = { ...current, extraCounts: { ...current.extraCounts }, extraValues: { ...current.extraValues } };
        ...
        for (const item of group.items) {
          ...
          const catItem = catalog.find(c => c.name.toLowerCase() === item.servico.toLowerCase());
