...
import { MESSAGE_CAMPAIGN_COLLECTIONS, MESSAGE_CAMPAIGN_SCHEMA_VERSION, maskMessagePhone, type MessageCampaignDocument, type MessageCampaignRecipientDocument } from "./src/services/messageCampaignSchema";
...
export function planMessageCampaignCreation(input: CreateMessageCampaignInput): MessageCampaignCreationPlan {
  ...
  const recipients = contacts.map((phone) => {
    ...
    return {
      ...
      leaseOwner: null,
      leaseExpiresAt: null,
      personalizedMessage: materializeMessage(input.message.trim(), {
  ...
  return {
    campaign: {
      ...
      sessionOwnerId: input.sessionOwnerId || input.createdBy,
      updatedAt: createdAt,
...
export async function createMessageCampaignAtomically(db: FirestoreLike, input: CreateMessageCampaignInput) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    return {
      campaign,
      recipientsCreated: eligibleRecipients.length,
      reused: false,
    };
  });
}

export const createMessageCampaign = (db: Firestore, input: CreateMessageCampaignInput) => createMessageCampaignAtomically(db as unknown as FirestoreLike, input);
