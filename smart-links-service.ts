...
const isSimulationAllowed = (user: VerifiedFirebaseUser | undefined) => {
  const isDev = process.env.NODE_ENV !== 'production';
  const explicitAdmin = process.env.ENABLE_ADMIN_DEMO_CONTROLS === 'true';
  return isDev || (explicitAdmin && user?.role === 'ADMIN');
};
...
export async function smartLinkRedirectHandler(req:express.Request,res:express.Response){
    try{
      const links=await allLinks();const link=findSmartLinkByCode(links,routeParam(req.params.code),new Date());
      ...
      const resolution=resolveSmartLink(link,new Date(),routeParam(req.params.code));
