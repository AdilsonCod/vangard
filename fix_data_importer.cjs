...
const badBlockStart = code.indexOf('} else if (importType === "CASHBARBER_PRODUTOS" && cashbarberReport) {');
const badBlockEnd = code.indexOf('} else if (importType === "CATALOGO") {', badBlockStart);
if (badBlockStart !== -1 && badBlockEnd !== -1) {
  code = code.substring(0, badBlockStart) + code.substring(badBlockEnd);
}

// Now insert the save block inside handleSave.
const handleSaveStr = `} else if (importType === "CATALOGO") {
          let currentCatalog = [...catalog];`;
          
const correctSaveStr = `} else if (importType === "CASHBARBER_PRODUTOS" && cashbarberReport) {
...
code = code.replace(handleSaveStr, correctSaveStr);
