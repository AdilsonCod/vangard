...
const actionLabels: Record<string,string> = { login_sucesso:'Login realizado', login_falha:'Tentativa de login falhou', criação_registro:'Registro criado', exclusao_registro:'Registro excluído', alteracao_campo:'Registro alterado', CREATED:'Registro criado', UPDATED:'Registro alterado', DELETED:'Registro excluído', CLOSED:'Caixa fechado', REOPENED:'Caixa reaberto', IMPORTED:'Dados importados', CALCULATED:'Cálculo realizado', TRANSFERRED:'Transferência registrada' };
...
export default function AuditHistory() {
  ...
  const [dataRows,setDataRows] = useState<AuditRow[]>([]);
  ...
  useEffect(() => {
    ...
    let pending = 2;
    const loaded = () => { pending -= 1; if (pending === 0) setBusy(false); };
    const failed = (label:string, snapshotError:Error) => {
      console.error(`Erro ao carregar histórico de ${label}:`,snapshotError);
      setError('Não foi possível carregar todos os registros de auditoria. Verifique as permissões da conta de gerência.');
      loaded();
    };
    const unsubscribeLogins = onSnapshot(query(collection(db,'loginAudit'),orderBy('timestamp','desc'),limit(500)), snapshot => {
      setLoginRows(snapshot.docs.map(item => ({id:item.id,...item.data()} as AuditRow))); loaded();
    }, snapshotError => failed('logins',snapshotError));
    const unsubscribeData = onSnapshot(query(collection(db,'dataAudit'),orderBy('timestamp','desc'),limit(500)), snapshot => {
      setDataRows(snapshot.docs.map(item => ({id:item.id,...item.data()} as AuditRow))); loaded();
    }, snapshotError => failed('alterações',snapshotError));
    return () => { unsubscribeLogins(); unsubscribeData(); };
  },[currentUser?.role]);

  const changeRows = useMemo<AuditRow[]>(() => {
    const legacyRows = financialAuditEvents.map(event => ({ id:event.id, userId:event.actorId, userName:event.actorName, userRole:event.actorRole, action:event.action, timestamp:event.createdAt, module:event.entityType, recordId:event.entityId, beforeState:event.previousValue, afterState:event.newValue }));
    return [...dataRows, ...legacyRows];
  },[dataRows,financialAuditEvents]);
  ...
  const actions = source==='changes' ? ['criação_registro','alteracao_campo','exclusao_registro','CREATED','UPDATED','DELETED','CLOSED','REOPENED','IMPORTED','CALCULATED','TRANSFERRED'] : ['login_sucesso','login_falha'];
  ...
  return <div className="space-y-6 p-4 sm:p-6 lg:p-8">
    <header className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900"><div className="flex items-center gap-3"><span className="rounded-xl bg-[var(--app-accent-soft)] p-3 text-[var(--app-accent)]"><History size={22}/></span><div><p className="text-xs font-black uppercase tracking-widest text-[var(--app-accent)]">Gerência</p><h1 className="text-2xl font-black">Histórico auditável</h1><p className="text-sm text-zinc-500">Logins e alterações de todos os módulos registrados em trilhas imutáveis.</p></div></div></header>
