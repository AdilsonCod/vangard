...
type SessionCredentials = { id: string; hash: string; environment: 'sandbox' | 'production'; webhookToken: string; publicToken: string };
...
export function CelcoinIntegration() {
  ...
  const [credentials, setCredentials] = useState<SessionCredentials>({ id: '', hash: '', environment: 'production', webhookToken: '', publicToken: '' });
  const [encryptedConfig, setEncryptedConfig] = useState<unknown>();

  useEffect(() => {
    getDoc(doc(db, 'integrationConfigs', 'celcoin')).then(async snapshot => {
      const saved = snapshot.data()?.encryptedConfig;
      if (!saved) return authenticatedApi.json<ConnectionStatus>('/api/celcoin?action=status').then(setConnection);
      setEncryptedConfig(saved);
      const result = await authenticatedApi.json<ConnectionStatus>('/api/celcoin?action=test', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ encryptedConfig: saved }) });
      setConnection(result);
      const synced = await authenticatedApi.json<Report>('/api/celcoin?action=transactions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ from: monthStart, to: today, limit: 500, encryptedConfig: saved }) });
      setReport(synced);
      setMessage(`Conexão restaurada e ${synced.total} transação(ões) sincronizada(s) automaticamente.`);
  ...
  const testConnection = async () => {
    ...
    try {
      const selectedCredentials = credentials.id && credentials.hash ? credentials : undefined;
      const result = await authenticatedApi.json<ConnectionStatus>('/api/celcoin?action=test', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(selectedCredentials ? { credentials: selectedCredentials } : { encryptedConfig }),
      });
  ...
  const saveConfiguration = async () => {
    if (currentUser?.role !== 'ADMIN') { setMessage('Somente a gerência pode alterar as credenciais.'); return; }
    setBusy(true); setMessage('');
    try {
      const encrypted = await authenticatedApi.json<{ encryptedConfig: unknown }>('/api/celcoin?action=encrypt', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ credentials }),
      });
      await setDoc(doc(db, 'integrationConfigs', 'celcoin'), { encryptedConfig: encrypted.encryptedConfig, updatedAt: new Date().toISOString(), updatedBy: currentUser.id });
      setEncryptedConfig(encrypted.encryptedConfig);
      const result = await authenticatedApi.json<ConnectionStatus>('/api/celcoin?action=test', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ encryptedConfig: encrypted.encryptedConfig }) });
      setConnection(result); setCredentials(value=>({...value,hash:'',webhookToken:'',publicToken:''})); setShowConfiguration(false);
      setMessage('Configuração salva e conexão autenticada. Ela será carregada automaticamente nos próximos acessos.');
  ...
  const loadReport = async () => {
    ...
    try {
      const result = await authenticatedApi.json<Report>('/api/celcoin?action=transactions', {
        ...
        body: JSON.stringify({ from, to, limit: 500, encryptedConfig }),
      });
  ...
  return <div className="space-y-5">
    <AppPageHeader eyebrow="Integração financeira" title="Celcoin · Galax Pay" description="Consulte transações e previsão de recebíveis para usar nas conciliações." icon={<WalletCards className="h-6 w-6" />} actions={currentUser?.role==='ADMIN'?<button onClick={()=>setShowConfiguration(true)} className="rounded-xl bg-[var(--theme-color)] px-4 py-2.5 text-sm font-bold text-white"><Settings className="mr-2 inline h-4 w-4"/>Configurar conexão</button>:undefined} />
    ...
    <section className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
      ...
      {!connection?.configured && <p className="mt-4 rounded-xl bg-amber-50 p-3 text-xs text-amber-800 dark:bg-amber-950/30 dark:text-amber-300">Use “Configurar conexão” para salvar as credenciais criptografadas e ativar a sincronização automática.</p>}
    </section>
    ...
    {showConfiguration && <div className="fixed inset-0 z-[200] grid place-items-center bg-black/70 p-4" onMouseDown={event=>{if(event.target===event.currentTarget)setShowConfiguration(false)}}><section role="dialog" aria-modal="true" aria-labelledby="celcoin-config-title" className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl dark:bg-zinc-900"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase text-[var(--theme-color)]">Credenciais da API</p><h2 id="celcoin-config-title" className="text-xl font-black">Configurar conexão Celcoin</h2></div><button aria-label="Fechar" onClick={()=>setShowConfiguration(false)} className="rounded-lg p-2 hover:bg-gray-100 dark:hover:bg-zinc-800"><X className="h-5 w-5"/></button></div><p className="mt-2 text-xs text-gray-500">As credenciais serão criptografadas no servidor e carregadas automaticamente nos próximos acessos.</p><div className="mt-5 space-y-4"><label className="block text-xs font-bold">Ambiente<select className={`${appControlClass} mt-2 w-full`} value={credentials.environment} onChange={e=>setCredentials(value=>({...value,environment:e.target.value as SessionCredentials['environment']}))}><option value="production">Produção</option><option value="sandbox">Sandbox</option></select></label><label className="block text-xs font-bold">Galax ID<input className={`${appControlClass} mt-2 w-full`} inputMode="numeric" autoComplete="off" value={credentials.id} onChange={e=>setCredentials(value=>({...value,id:e.target.value}))} placeholder="Informe o Galax ID"/></label><label className="block text-xs font-bold">Galax Hash<input className={`${appControlClass} mt-2 w-full`} type="password" autoComplete="new-password" value={credentials.hash} onChange={e=>setCredentials(value=>({...value,hash:e.target.value}))} placeholder="Informe o Galax Hash"/></label><label className="block text-xs font-bold">Token de segurança do webhook<input className={`${appControlClass} mt-2 w-full`} type="password" autoComplete="new-password" value={credentials.webhookToken} onChange={e=>setCredentials(value=>({...value,webhookToken:e.target.value}))} placeholder="Informe o token do webhook"/></label><label className="block text-xs font-bold">Public Token<input className={`${appControlClass} mt-2 w-full`} type="password" autoComplete="new-password" value={credentials.publicToken} onChange={e=>setCredentials(value=>({...value,publicToken:e.target.value}))} placeholder="Informe o Public Token"/></label><div className="rounded-xl bg-gray-50 p-3 text-xs dark:bg-zinc-800"><span className="font-bold">URL do webhook</span><code className="mt-1 block break-all">{`${window.location.origin}/api/celcoin-webhook`}</code></div></div><button disabled={busy||!credentials.id.trim()||!credentials.hash.trim()||!credentials.webhookToken.trim()||!credentials.publicToken.trim()} onClick={saveConfiguration} className="mt-5 w-full rounded-xl bg-[var(--theme-color)] px-4 py-3 font-bold text-white disabled:opacity-50"><RefreshCw className="mr-2 inline h-4 w-4"/>{busy?'Salvando e testando…':'Salvar configuração e conectar'}</button></section></div>}
  </div>;
}
