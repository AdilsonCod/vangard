...
import { CheckCircle2, CloudDownload, Database, RefreshCw, ShieldCheck, WalletCards } from 'lucide-react';
...
export function CelcoinIntegration() {
  ...
  const testConnection = async () => {
    ...
    try {
      const result = await authenticatedApi.json<ConnectionStatus>('/api/celcoin?action=test', { method: 'POST' });
  ...
  const loadReport = async () => {
    setBusy(true); setMessage('');
    try {
      const params = new URLSearchParams({ action: 'transactions', from, to, limit: '500' });
      const result = await authenticatedApi.json<Report>(`/api/celcoin?${params}`);
  ...
  return <div className="space-y-5">
    <AppPageHeader eyebrow="Integração financeira" title="Celcoin · Galax Pay" description="Consulte transações e previsão de recebíveis para usar nas conciliações." icon={<WalletCards className="h-6 w-6" />} />
    ...
    <section className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
      ...
      {!connection?.configured && <p className="mt-4 rounded-xl bg-amber-50 p-3 text-xs text-amber-800 dark:bg-amber-950/30 dark:text-amber-300">Configure CELCOIN_GALAX_ID, CELCOIN_GALAX_HASH e CELCOIN_ENVIRONMENT no ambiente seguro do servidor.</p>}
    </section>
