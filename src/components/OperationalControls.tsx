...
export default function OperationalControls({ kind, selectedUnit, strictUnitScope = false }: Props) {
  ...
  const [form, setForm] = useState({ unitId: selectedUnit === 'ALL' ? '' : selectedUnit, date: today, clientName: '', barberId: '', catalogItemId: '', itemName: '', amount: '', status: 'PENDENTE' as FinancialTransaction['status'] });
  ...
  const suggestions = catalog.filter(item => {
    if (isCourtesy && String(item.type).toLowerCase().includes('product')) return false;
    return !form.unitId || !item.unit || item.unit === 'ALL' || item.unit === form.unitId;
  });
  ...
  const newRecord = () => {
    setEditing(null);
    setForm({ unitId: selectedUnit === 'ALL' ? units[0]?.id || '' : selectedUnit, date: period === today.slice(0, 7) ? today : `${period}-01`, clientName: '', barberId: '', catalogItemId: '', itemName: '', amount: '', status: 'PENDENTE' });
    setOpen(true);
  };
  const editRecord = (item: FinancialTransaction) => {
    setEditing(item);
    const catalogItem = catalog.find(catalogEntry => catalogEntry.id === item.catalogItemId)
      || catalog.find(catalogEntry => catalogEntry.name === (item.itemName || item.description));
    setForm({ unitId: item.unitId, date: item.date, clientName: item.clientName || '', barberId: item.barberId || '', catalogItemId: catalogItem?.id || '', itemName: item.itemName || item.description, amount: item.amount.toFixed(2).replace('.', ','), status: item.status });
    setOpen(true);
  };
  const save = async () => {
    const amount = parseMoney(form.amount);
    if (!form.unitId || !form.date || !form.barberId || !form.itemName.trim() || (!isCourtesy && !form.catalogItemId) || (isCourtesy && !form.clientName.trim()) || amount <= 0) return window.alert(!isCourtesy && form.catalogItemId && amount <= 0 ? 'Cadastre o preço de custo deste item no Catálogo antes de lançar a venda interna.' : 'Preencha todos os campos obrigatórios e informe um valor maior que zero.');
  ...
  const remove = async (item: FinancialTransaction) => {
    if (await confirmAction({ title: 'Excluir registro', description: 'Deseja excluir este registro permanentemente? Os lançamentos relacionados em Pagamentos também serão atualizados.', confirmText: 'Excluir registro' })) {
      try {
